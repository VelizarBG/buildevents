...
public class BuildEventCommand {
	...
	private static int setEventWorld(ServerCommandSource source, String eventName, @Nullable ServerWorld world) throws CommandSyntaxException {
		...
		if (event.world() == world) {
			...
			buildEventsState.markDirty();
			if (world == null) {
				source.sendFeedback(new TranslatableText("commands.buildevents.set.world.global", eventName), true);
			} else {
				source.sendFeedback(new TranslatableText("commands.buildevents.set.world.success", world.getRegistryKey().getValue().toString(), eventName), true);
			}
			return 1;
		}
	}
	...
	private static int setEventTotal(ServerCommandSource source, String eventName, boolean total) throws CommandSyntaxException {
		...
		if (event.total() == total) {
			...
			if (total) {
				...
				source.sendFeedback(new TranslatableText("commands.buildevents.set.total.true", eventName), true);
			} else {
				...
				source.sendFeedback(new TranslatableText("commands.buildevents.set.total.false", eventName), true);
			}
			return 1;
		}
	}
